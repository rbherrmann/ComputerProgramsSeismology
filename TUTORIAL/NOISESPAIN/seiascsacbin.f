	...
	read(5,'(a)')infile
	write(6,'(a)')infile
	read(infile,'(30x,i3,i3,1x,i3,1x,i2,1x,i2,1x,i2,1x,i2,1x,f6.3,1x,f9.3)')ntrace,iyear,idoy,imon,iday,ihr,imn,sec,twin
	iyear=iyear+1900
	write(6,'(i4,1x,i3.3,1x,i2.2,1x,i2.2,1x,i2.2,1x,i2.2,1x,f6.3,1x,f9.3)')iyear,idoy,imon,iday,ihr,imn,sec,twin

	read(5,'(a)')infile
	write(6,'(a)')infile

	do i=1,10 + (ntrace/3) -1
		read(5,'(a)')infile
		write(6,'(a)')infile
	enddo
c-----
c	get the individual traces
c-----
	do i=1,ntrace
C		read(5,'(a)')header
c-----
c		get the trace head - this is 1040 characters read in 80 character
c		groups
c-----
		do ii=1,13
			il = (ii-1)*80 + 1
			ih = ii*80
			read(5,'(a)')header(il:ih)
			write(6,'(a)')header(il:ih)
		enddo
			read(header,13)stnm,cmp,iyear,idoy,imon,iday,ihr,imn,sec,rate,npts,slat,slon,stel
   13 format(a5,a4,i3,1x,i3,1x,i2,1x,i2,1x,i2,1x,i2,
     1			1x,f6.3,1x,f7.2,i7,1x,f8.4,1x,f9.4,1x,i5)
		iyear=iyear+1900
			write(6,*)stnm,cmp,iyear,idoy,imon,iday,ihr,imn,sec,rate,npts,slat,slon,stel
		if(stnm(1:1).eq.' ')stnm(1:1)='_'
		if(stnm(2:2).eq.' ')stnm(2:2)='_'
		if(stnm(3:3).eq.' ')stnm(3:3)='_'
		if(stnm(4:4).eq.' ')stnm(4:4)='_'
		if(stnm(5:5).eq.' ')stnm(5:5)='_'
		if(cmp(1:1).eq.' ')cmp(1:1)='_'
		if(cmp(2:2).eq.' ')cmp(2:2)='_'
		if(cmp(3:3).eq.' ')cmp(3:3)='_'
		if(cmp(4:4).eq.' ')cmp(4:4)='_'

		WRITE(6,*)'READ THE DATA:',NPTS
		read(5,'(7i11)')(iarr(j),j=1,npts)
		do j=1,npts
			x(j) = iarr(j)
		enddo
		write(6,*)(x(j),j=1,20)
		write(6,*)(x(j),j=NPTS-19,NPTS)

		ofile=stnm//cmp//'.sac'
		dt = 1.0/rate
		call newhdr()
		call setnhv('NZYEAR', iyear, nerr)
		call setnhv('NZJDAY', idoy, nerr)
		call setnhv('NZHOUR', ihr, nerr)
		call setnhv('NZMIN', imn, nerr)
		isec = sec
		imsec = sec - isec
		call setnhv('NZSEC', isec, nerr)
		call setnhv('NZMSEC', imsec, nerr)
		kstnm=' '
		kcmpnm = ' '
		kstnm = stnm
		kcmpnm = cmp
		...
		call wsac1(ofile,x,npts,0.0,dt,nerr)
	enddo
